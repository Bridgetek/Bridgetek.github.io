...
This application note describes methods and options for graph drawing on the BT82x. 
It uses the [Eve-MCU-Dev](https://github.com/Bridgetek/Eve-MCU-Dev) EVE library for embedded MCUs and the [python-bt82x-dev](https://github.com/Bridgetek/python-bt82x-dev) library for python. 
...
| cmd_*() | Write 32 bits command with its necessary parameters to command FIFO i.e., RAM_CMD. |
| dl() | Write 32 bits display list command to RAM_DL. |
| host_command() | send host command to EVE |
...
| ![Simple BARGRAPH Example](docs/bargraph.png "Simple BARGRAPH Example")|
|-|
...
```
        ...
        VERTEX2F(10, 10)
```
The height component of the BITMAP_LAYOUT is set to 1 since that is the height of the data-set. The height component of the BITMAP_SIZE is set to 256 to encompass the full range of the data (i.e. 0 to 255).
...
To draw a line graph the natural primitive to use would be the LINE_STRIP method and VERTEX2F commands to draw lines between individual points on a chart. This requires that the display list is filled with commands to draw the line of the line graph.
...
| ![Simple LINE_STRIP Example](docs/linestrip.png "Simple LINE_STRIP Example")|
|-|
...
| ![Subtractive Overlay Example](docs/subtractive.png "Subtractive Overlay Example")|
|-|
...
| ------------- |:-------------:| -----:|
| 1.0 | Initial Release | 16/04/2025 | 
